...
> **現状のまま提供される。作者は一切の責任を負わない。
> 使った時点で、これに同意したものとする。**
>
> インストーラはディスクを丸ごと消す選択肢を持っている。消えて困るものは
> 先に別の場所へ写しておくこと。詳しくは [`LICENSE.ja.md`](LICENSE.ja.md)。
> myOS 自身は MIT ([`LICENSE`](LICENSE))。同梱している Linux カーネルや
> Debian のパッケージはそれぞれのライセンスに従う。
