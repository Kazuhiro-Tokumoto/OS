...
def main():
    ...
    # ログは黙らせない。
    #
    # 以前は loglevel=3 を付けていた。インストーラの画面がカーネルの
    # ログで上書きされるのを防ぐためだったが、これのせいで
    # 「ブートローダーの画面は出たのに、そのあと真っ暗」という
    # 起動失敗が、何も手掛かりの無いただの黒画面になっていた。
    # インストーラは自分で画面を消してから描くので、上書きは実害が無い。
    # 起動しなかったときに理由が見えることのほうが大事。
    #
    # 黙らせたい場合は --cmdline で loglevel=3 を付けて渡せる。
