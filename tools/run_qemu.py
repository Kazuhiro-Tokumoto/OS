...
def parse_keys(spec: str):
    ...
    # shift を押しながら出す記号。
    #
    # 注意: sendkey が送るのはスキャンコードなので、
    # どの文字が出るかはゲスト側のキーボード配列で決まる。
    # ここの対応は US 配列を前提にしている。
    # 日本語配列で動かしていると shift-9 は "(" ではなく ")" になる、
    # といったずれ方をする。記号を打たせる検証を書くときは、
    # 配列によって変わらない文字 (英数字 / . / / / -) に寄せると安全。
    shifted = {
        "!": "1", "@": "2", "#": "3", "$": "4", "%": "5", "^": "6",
        "&": "7", "*": "8", "(": "9", ")": "0", "_": "minus",
