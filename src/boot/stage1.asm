...
; (BIOS Parameter Block) があるはずだ」と決めつけて、そこへ media の
; ジオメトリを書き込んでから制御を渡すものがある。書き込む先は
;
;   0x18  1 トラックあたりのセクタ数
;   0x1A  ヘッド数
;   0x1C  隠しセクタ数
...
;
; ここにコードを置いていると、**起動する前に自分のコードを壊される**。
; CD (El Torito) では起きないので、USB でだけ出る。原因が見えにくい。
;
; syslinux の isohybrid は 0x02-0x1F を NOP で埋めて、踏まれても滑って
; 0x20 の本体に着くようにしている (Alpine と Debian の ISO を実際に
; 読んで確かめた)。ただしそれだと全域を書き換える BIOS には勝てない。
;
; myOS は FAT のブートセクタと同じ形にして、**0x03-0x3D を丸ごと明け
; 渡す**。先頭で飛び越すので、そこに何を書かれても関係ない。
;
; 中身は 0 のままにして、FAT だと名乗らない。ここに本物らしい BPB を
; 置くと、BIOS によっては「フロッピーだ」と判断して FDD エミュレーション
; を選ぶ。こちらはパーティションテーブルを持っているので、
; ハードディスクとして扱ってもらったほうが都合がよい。
        jmp     short start
        nop
        times 0x3E-($-$$) db 0
