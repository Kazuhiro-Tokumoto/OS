...
| `Win+←` / `Win+→` | 画面の左半分 / 右半分にスナップ |
| `Win+↑` | 最大化 |
| `Win+↓` | 元に戻す（最大化していなければ最小化） |

`Win+D` は「自分が引っ込めた窓」だけを覚えておいて、
もう一度押したときにそれだけを戻す。
最初から最小化されていた窓を勝手に開かないようにするため。

`Win+←` / `Win+→` のスナップは Windows 7 以降のあれ。
元の大きさは `old_x` などに退避してあるので、
`Win+↓` や最大化ボタンで戻せる。
