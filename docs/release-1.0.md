...
| | |
| --- | --- |
| **画面が真っ黒になる** | `nomodeset` を既定に。DRM ドライバ (vmwgfx など) がブートローダーの用意した画面を奪い、何も映らなくなっていた |
...
解像度はブートローダーが VBE で決めて、その画面をそのまま使い続ける
作りにしている（DRM に触らせない = `nomodeset`）。X からは変えられない
ので、「次に起動するときの希望」をディスクに書いて再起動する形になる。
...
| グラフィックス | **VBoxVGA**（VMSVGA 以外） | `vmwgfx` が画面を奪い、何も映らなくなる |
| メモリ | 2 GB 以上 | インストーラが 512 MB の tmpfs を使うので 1 GB では足りない |

64bit でない場合は Stage2 が理由を画面に出して止まる。
グラフィックスのほうは `nomodeset` を既定で渡すようにしたので、
1.0 より後のイメージでは VMSVGA のままでも映る。
