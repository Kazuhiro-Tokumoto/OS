...
> **セキュリティとアプリ用ランタイムを入れて 1.6 GiB → 2.2 GiB になった。**
> 増えたぶんの内訳は下の「増えたもの」を参照。



`du -sm` の実測で **2.2 GiB**。
4 GiB のパーティションに対して、空きが 1.8 GiB ほど残る。

### 増えたもの

| 中身 | 増えた量 | なぜ |
| --- | --- | --- |
| OpenJDK 17 **JDK** | +110 MiB | JRE だと `.java` をコンパイルできない |
...
| 圧縮したルート (squashfs / zstd) | **827 MiB**（実測: tar + zstd -8 で 867,363,857 バイト） |
| ISO9660 + El Torito のオーバーヘッド | 1 MiB 前後 |
| **合計** | **約 860 MiB** |

**CD-R 1 枚 (700 MiB) には収まらなくなった。**
USB メモリ (1 GiB 以上) か DVD-R を使う。

CD-R に戻したい場合は、次のどれかを削ることになる。

| 削るもの | 圧縮後で減る量 | 失うもの |
| --- | --- | --- |
| ClamAV の `main.cvd` | -85 MiB | 初回起動時にネットから取るまで検査できない |
| JDK を JRE に戻す | -40 MiB | `.java` のコンパイル |
| `fonts-noto-cjk` | -40 MiB | 日本語・中国語・韓国語の表示 |
| gcc + ヘッダ | -30 MiB | 本体の上でのビルド |
| locale を日英だけに | -20 MiB | 他の言語 |

全部やれば 650 MiB 前後まで下がるが、
**USB メモリで配るなら削る意味は薄い。**
